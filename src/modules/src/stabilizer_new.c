...
STATIC_MEM_TASK_ALLOC(stabilizerNewTask, STABILIZER_NEW_TASK_STACKSIZE);

static void stabilizerNewTask(void* param);
...
void stabilizerNewInit()
{
  ...
  STATIC_MEM_TASK_CREATE(stabilizerNewTask, stabilizerNewTask, STABILIZER_NEW_TASK_NAME, NULL, STABILIZER_NEW_TASK_PRI);
...
static void stabilizerNewTask(void* param)
{
  ...
  vTaskSetApplicationTaskTag(0, (void*)TASK_STABILIZER_NEW_ID_NBR);
...
void stabilizeNewSetEmergencyStop()
{
  emergencyStop = true;
}

void stabilizerNewResetEmergencyStop()
{
  emergencyStop = false;
}
